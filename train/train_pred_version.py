...
def _plan_path(base):
    return os.path.join(base, "all_plans_by_hybrid_random_on_graph.json")
...
def load_training_data(training_data_file, template_id):
    ...
    for param_key in param_keys:
        ...
        candidate_plan = random.sample(list(cost[param_key].keys()), min(k1[3], len(cost[param_key].keys())))
...
def alternating_training(training_data, model_path, device, epochs, epoch_step):
    # all_folders = ['20a', '22a', '32a', '29a', '1a', '10a', '3a', '26a', '25a', '30a', '16a', '27a', '19a', '5a']
    all_folders = []
    for subdir, _, files in os.walk(training_data):
        if ('a' in os.path.basename(subdir) and "meta_data.json" in files and "all_plans_by_hybrid_new.json" in files
                and "parameter_new.json" in files and "latency_matrix_pg.json" in files):
...
if __name__ == "__main__":
    ...
    if function == "training_baseline":
        ...
        alternating_training(training_data, model_path, args.device, 50, 1)
